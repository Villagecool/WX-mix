function onCreate()
	for i = 0, getProperty('unspawnNotes.length')-1 do
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'BendyNote_InkHurt' then
			setPropertyFromGroup('unspawnNotes', i, 'texture', 'NOTE_InkKill_assets');
...
function goodNoteHit(id, noteData, noteType, isSustainNote)
	if noteType == 'BendyNote_InkHurt' then
		addMisses(1);
   		addScore(-500);
